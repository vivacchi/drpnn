...
ユーザー判断 (2026-05-25 議論): 入出力数 制約 (入力 20 / 出力 30) と
規模拡張の希望から、 **柱状 grid 5×6×22 = 660 セル** を採用。

**理由**:
- 上面 5×6 = 30 セル = 出力 30 ぴったり (完全充填)
- 底面 5×6 = 30 セル = 入力 20 + 内部 10 (B1 中央寄せ)
- 内部層 20 層 × 30 = 600 セルで minicolumn 的縦構造
- 計算量 2D 440 の 1.5x (実用範囲)
- DRP 1 PE 時分多重で論理 3D を物理 2D PE に写像可能

段階的実装:
- Step 7: 小規模プロトタイプ (5×6×5 = 150) で動作確認
- Step 8: 本実装 (5×6×22 = 660) で評価

### 2.4 入出力配置 (最終決定: 柱状 5×6×22)

ユーザー要望: 入力 20 / 出力 30 (= 入力 < 出力)、 底面入力 / 上面出力 でバランス良く配置。

**採用設計: grid 5×6×22 = 660 セル (柱状)**

底面と上面が 5×6=30 セルでぴったり対称、 上面 30 = 出力数で完全充填:

```
grid 5 × 6 × 22 = 660 セル

z=0 (底面、 30 セル):
  入力 20: x=0..4, y=1..4 (5 × 4 = 20、 中央寄せ B1 配置)
  内部 10: y=0 行 5 セル + y=5 行 5 セル

z=1..20 (内部 20 層、 600 セル):
  抑制 ~108 (18%) + 興奮 ~492 (82%)、 ランダム配置 (seed ベース)

z=21 (上面、 30 セル):
  出力 30 (全セル使い切り、 1 セル 1 ニューロン)
...
| 観点 | 決定 |
|---|---|
| 入力配置 (中央 vs ランダム) | **中央寄せ (B1)**: 構造明瞭、 minicolumn 内に集約 |
| 出力配置 | **上面全 (5×6=30 完全)**: ぴったり、 余り無し |
| 内部層配置 | ランダム (seed ベース)、 興奮 / 抑制 18% で M1 2D と同じ |
| Z 軸長 | **22**: 内部層 20 で minicolumn 厚みを表現 |
| 計算量比 (2D 440 比) | **1.5x** (660/440) |
