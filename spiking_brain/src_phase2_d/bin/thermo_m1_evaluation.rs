...
use spiking_brain::phase2_d::thermo_network::{ThermoNetwork, ThermoNetworkConfig};
...
fn main() {
    ...
    let snap_interval: usize = if n_train >= 1000 { 1000 } else { (n_train / 10).max(10) };
    ...
    println!("== Phase 2 Fork D (全結線初期 conductance=50) ホワイトノイズ訓練版 ==");
    ...
    let mut csv_snap = File::create("phase2_d_whitenoise_10m_snapshots.csv").expect("snap csv");
