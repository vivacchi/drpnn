...
/// Fork B: 新生結線の保護期間 (step)。
/// この期間 decay() はスキップされる。生物の GAP-43 期間 (新生 synapse の維持期間) に対応。
/// 1000 step ≈ 1.67 試行 (300ms/0.5ms)
pub const MATURATION_PERIOD: i32 = 1000;
...
pub struct ThermoSynapse {
    ...
    /// Fork B: 成熟までの残りステップ数。>0 の間は decay() がスキップされる。
    /// 軸索成長で生まれた瞬間に MATURATION_PERIOD で初期化、毎 step -1。
    pub maturation_remaining: i32,
}

impl ThermoSynapse {
    /// 新規プラスチック結線 (軸索成長で作られるもの)
    /// Fork B: 成長時 maturation_remaining = MATURATION_PERIOD で初期化
    pub fn new_plastic(pre: usize, post: usize, delay: i32, conductance: i32, is_inhibitory: bool) -> Self {
        Self {
            ...
            plastic: true,
            decay_counter: 0,
            maturation_remaining: MATURATION_PERIOD,
        }
    }
    ...
    pub fn new_fixed(pre: usize, post: usize, delay: i32, conductance: i32, is_inhibitory: bool) -> Self {
        Self {
            ...
            plastic: false,
            decay_counter: 0,
            maturation_remaining: 0, // 固定結線は保護不要
        }
    }
    ...
    /// Fork B: maturation_remaining > 0 の間は減衰スキップ (新生結線の保護)。
    pub fn decay(&mut self) {
        if !self.plastic { return; }
        // 保護期間中は減衰しない (毎 step カウントダウン)
        if self.maturation_remaining > 0 {
            self.maturation_remaining -= 1;
            return;
        }
