...
pub struct ThermoNetworkConfig {
    ...
    /// Fork E: cortex 内の plastic 結線 fanout
    /// 各 cortex_exc neuron が他 cortex に張る初期 plastic 結線本数。
    /// 発生学的に妥当な過剰接続: 40 (320 exc × 40 = 12800 本)。
    /// 生物の発達期シナプス過形成 → STDP + decay による刈り込み を物理シミュレート。
    pub cortex_plastic_fanout: usize,
    /// Fork E: cortex 内 plastic 結線の初期 conductance
    /// OPEN_THRESHOLD=30 ギリギリで生まれる (exists=true、信号は弱い 3)
    pub cortex_plastic_initial_conductance: i32,
...
impl Default for ThermoNetworkConfig {
    fn default() -> Self {
        Self {
            ...
            input_fanout: 80,
            // Fork E: 過剰接続。生物発達のシナプス過形成期に相当
            cortex_plastic_fanout: 40, // 320 × 40 = 12800 本の plastic 結線
            cortex_plastic_initial_conductance: 30, // OPEN_THRESHOLD ギリギリ
            delay_range: (2, 40),
            seed: 300,
...
impl ThermoNetwork {
    pub fn new(config: ThermoNetworkConfig) -> Self {
        ...
        // (7) Fork E 過剰接続: cortex_exc neuron が他 cortex に大量の plastic 結線を張る
        //     発生学的妥当性: 生物の発達期シナプス過形成 (12800 本程度)
        //     初期 conductance = 30 (OPEN_THRESHOLD ギリギリ、exists=true で生まれる)
        //     STDP で強化されるものは残り、使われないものは decay で消える
        //     軸索成長機構は引き続き機能 (成体可塑性として補助的)
        if config.cortex_plastic_fanout > 0 {
            // cortex_exc のインデックスを再計算 (exc_indices は既にあるが範囲を明示)
            let cortex_exc_indices: Vec<usize> = neurons.iter().enumerate()
                .filter(|(i, n)| !n.is_inhibitory && !input_neurons.contains(i))
                .map(|(i, _)| i)
                .collect();
            // 全 cortex (exc + inh) を target 候補に
            let cortex_all: Vec<usize> = (config.n_input..neurons.len()).collect();
            let init_cond = config.cortex_plastic_initial_conductance;
            for &pre in &cortex_exc_indices {
                let mut cand: Vec<usize> = cortex_all.iter().copied().filter(|&t| t != pre).collect();
                cand.shuffle(&mut rng);
                for &post in cand.iter().take(config.cortex_plastic_fanout) {
                    let d = rng.gen_range(delay_dist.0..=delay_dist.1);
                    synapses.push(ThermoSynapse::new_plastic(pre, post, d, init_cond, false));
                }
            }
        }
