...
最終更新: 2026-05-31 (蝸牛 解像度倍化 + mo バグ修正、 sel 0.581 で固定パターン上限の 80%)

## 直近の状態 (2026-05-31)

### 蝸牛 解像度倍化の前進 (本日の成果、 PAPER §5.15)
- **M1 音素 selectivity が 0.537 → 0.581** に (固定パターン上限 0.728 の **80%**)
- 構成: **40 帯域 蝸牛 + 84ch 蝸牛神経核 + speed=3 + decay_slow=3**
  - bin: `m0_cn_m1_pipeline -- 5000 3 3` (新しい `for_m1_cn_40()` 使用)
  - grid 20×26 = 520 neurons、 シナプス 22,740
  - active 18/40、 全音素応答 (16-18 hits 均一)
- 蝸牛 ki-se cosine が **0.692 → 0.355** に大幅改善 (狙い通り)
- ただし M1 出力 ki-se cosine は **0.908 と高い** (蝸牛で開いた分化を M1 が再収束)
- 最大ボトルネック: **ki-tu = 0.966** (両 Plosive、 M1 で完全混同 → M1.5/M2 課題)

### mo 音素バグの発見・修正
- Nasal 振幅 [3000, 1500] (peak 4500) で蝸牛 firing threshold 未満 → 全帯域取りこぼし
- 修正: amps [6000, 3000] (peak 9000、 母音と同等)
- mo が 4 帯域 (ch 7, 8, 9, 14) で発火するように改善 (旧は ch 14 のみ)
- ※ 5K 訓練は修正前に実施したため、 mo fix 適用版での再訓練は未
...
- **M1 A1** (ThermoNetwork): `for_m1_cn_40` (84入力) で sel 0.581
...
### 設計知見 (本日獲得)
- 蝸牛 解像度 倍化は **小さな前進** (+0.044 sel) — 直線的にスケールしない
- 散逸構造の平衡点は「入力統計 × 時間構造 × 可塑性時定数 × **入力解像度**」 の組合せ
- ki-tu 100% 混同は音響特性の本質的類似 → M1 単独では解消困難 (上位処理必須)

## 次のタスク (優先度順)

1. **mo fix 適用版 5K 再訓練** — mo の 4 帯域化が selectivity をどこまで押すか
2. **M1.5 皮質間時間処理ステージ** — M1→M2 にオンセット検出器を入れ M2 collapse 解消、 ki-tu 区別の上位処理にもなる
3. **解像度 60/80 帯域の収益逓減ライン** — どこで頭打ちか
4. **DRAM 参照ランプ ADC を DramNetwork に統合** — graded spike 実現
5. **note 記事化** — 「蝸牛 解像度倍化 + mo バグ発見」 (5 本目)
