...
DESIGN_PHILOSOPHY.md / CONTEXT.md で示された 12 モジュール構成のうち、**最下層の感覚変換モジュール M0 蝸牛** を設計する。これは「音声波形 → 時間構造をもつスパイク列」への決定論的変換装置であり、M1 (一次聴覚野) への入力を供給する。
...
**両耳間時間差 (ITD)** で音源定位:
- 数 μs (~10) の精度
- 周波数情報ではなく波形のゼロクロス時刻が情報源
- これが phase locking の存在理由

聴覚は **周波数の精度よりも時間の精度** が高い。これが「時間 bin 化評価」の正当性を示す本質的根拠。
...
### 3.6 自発発火 (spontaneous activity)

各 step で各 input neuron が決定論的に低頻度で発火:
```
spontaneous_counter[i] += 1;
if spontaneous_counter[i] >= SPONTANEOUS_INTERVAL[i] {
    spontaneous_counter[i] = 0;
    fire(input_neuron[i]);
}
SPONTANEOUS_INTERVAL[i] = 200 + (i * 11) % 137  // 個体差ありの周期
```

`% 137` (素数) で同期しないように。これは決定論的だが見かけ上「random」な発火パターン。
...
1. **Step 1**: 整数 biquad IIR フィルタの実装 + テスト (周波数応答確認)
2. **Step 2**: 包絡線検出 + 圧縮 + 閾値発火 (1 チャンネル動作確認)
3. **Step 3**: 20 帯域に拡張 + 自発発火 (ホワイトノイズ音波で発火パターン確認)
4. **Step 4**: 音素生成器 (5 母音 + 5 子音)
5. **Step 5**: M0 + M1 統合 (5 音節 5 sample で発火応答確認)
6. **Step 6**: 10k 訓練 + 時間 bin 化評価 (M1 単体と比較)
...
聴神経の自発発火は 50-100 Hz だが、これを M1 の DT_MS = 0.5 ms / step 系に落とすと:
- 1 step = 0.5 ms、100 Hz = 10 ms 周期 = 20 step 周期
- 各 input neuron が 20 step ごとに発火 → ホワイトノイズ的 noise floor

これが M1 にとって「過剰刺激」にならないか要検証。
